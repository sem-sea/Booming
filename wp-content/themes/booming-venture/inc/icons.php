 ...
 * AI crawlers see the icon as part of the page.
 *
 * Usage in a pattern file:
 *   <?php echo bv_icon( 'rocket' ); ?>
 *   <?php echo bv_icon( 'rocket', 32, 'bv-service-icon-svg' ); ?>
 *
 * All icons share viewBox 0 0 24 24, stroke="currentColor", and
 * stroke-width 2 so they recolour via CSS `color` and scale crisply.
 *
...
function bv_icon( string $name, int $size = 24, string $class = '' ): string {
	$paths = bv_icon_paths( $name );
	if ( ! $paths ) return '';
	$classes = trim( 'bv-icon bv-icon-' . sanitize_html_class( $name ) . ' ' . $class );
	return sprintf(
...
function bv_icon_paths( string $name ): string {
	static $icons = null;
	if ( null === $icons ) {
		$icons = [
